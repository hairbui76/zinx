...
module.exports = {
	...
	showNginxAccessListForm: function (model) {
		console.log('showNginxAccessListForm called with model:', model);
		if (Cache.User.isAdmin() || Cache.User.canManage('access_lists')) {
			console.log('User has permission to manage access lists');
			try {
				require(['./main', './nginx/access/form'], function (App, View) {
					console.log('Form module loaded successfully');
					try {
						const formView = new View({model: model});
						console.log('Form view created successfully');
						App.UI.showModalDialog(formView);
						console.log('Modal dialog shown');
					} catch (innerErr) {
						console.error('Error creating form view:', innerErr);
					}
				});
			} catch (err) {
				console.error('Error loading form module:', err);
			}
		} else {
			console.log('User does not have permission to manage access lists');
		}
	},
