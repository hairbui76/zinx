...
const keysFile = '/data/zinx-gateway/keys.json';
...
const configure = () => {
	const filename = (process.env.NODE_CONFIG_DIR || '/data/zinx-gateway') + '/' + (process.env.NODE_ENV || 'default') + '.json';
	...
	logger.info('Using Sqlite: /data/zinx-gateway/database.sqlite');
	instance = {
		database: {
			engine: 'knex-native',
			knex: {
				...
				connection: {
					filename: '/data/zinx-gateway/database.sqlite',
				},
